...
def scrape_abs_cbn():
    ...
    combined_news = {item['title']: item for item in (latest_news + more_news)}.values()
    ...
    for news_item in combined_news:
        ...
        try:
            ...
            driver.get("https://news.abs-cbn.com")  # First, go to the main page
            ...
            print(date_element)
            if date_element:
                news_item['date'] = date_element['content']  # Extract the date content attribute
    ...
    tags_section = soup.find_all("div", class_="MuiGrid-root MuiGrid-item css-1t0fio2")
    ...
    for div in tags_section:
        tag_link = div.find("a", class_="MuiTypography-root MuiTypography-inherit MuiLink-root MuiLink-underlineNone css-cs6pc5")
...
def scrape_manila_times():
    ...
    # Find the latest news articles
    latest_news_section = soup.find("div", class_="widget-container hp-latest-stories")
    ...
    for news_item in latest_news_items:
        title = news_item.get('title', '').strip()  # Extract title from the 'title' attribute
        ...
        date_element = article_soup.find("div", class_="article-publish-time roboto-a")
